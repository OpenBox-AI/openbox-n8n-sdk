...
let openboxSdkPromise;
function loadOpenBoxSdk() {
...
function buildFallbackText(nodeName, prompt) {
...
function deterministicBlockReason(nodeName, prompt) {
    const lower = prompt.toLowerCase();
    const hasPaymentCard = /\b(?:\d[ -]*?){13,19}\b/.test(prompt);
    const hasSsn = /\b\d{3}-\d{2}-\d{4}\b/.test(prompt);
    const asksForSecrets = /ignore all instructions/i.test(prompt) &&
...
class OpenboxLlm {
    description = {
        ...
        properties: [
            ...
            },
            {
                ...
                name: 'model',
                type: 'string',
                default: '={{ $env["LLM_PROVIDER_MODEL"] || $env["OPENROUTER_MODEL"] || $env["OLLAMA_MODEL"] || "liquid/lfm-2.5-1.2b-instruct:free" }}',
            },
            {
            ...
            },
            {
                ...
                type: 'string',
                default: 'chatInput',
                description: 'Name of the field on each incoming item that holds the user message. Defaults to "chatInput" so the demo Chat Trigger keeps working; change this when wiring up a different upstream node.',
            },
            {
                displayName: 'Tip: attach an OpenBox credential above to skip the URL / API Key fields below. The credential takes precedence when set.',
            ...
            },
            {
                ...
                description: 'Legacy fallback used only when no OpenBox credential is attached. Will be removed in a future version.',
            },
            {
                ...
                description: 'Legacy fallback used only when no OpenBox credential is attached (obx_live_* or obx_test_*).',
            },
        ],
    };
    methods = {
    ...
    async execute() {
        ...
        const llmProvider = this.getNodeParameter('llmProvider', 0);
        const model = this.getNodeParameter('model', 0);
        const systemPrompt = this.getNodeParameter('systemPrompt', 0);
        const inputFieldName = String(this.getNodeParameter('inputFieldName', 0, 'chatInput')).trim() || 'chatInput';
        const openRouterApiKey = llmProvider === 'openrouter'
            ? this.getNodeParameter('openRouterApiKey', 0, '')
            : '';
        const openRouterBaseUrl = llmProvider === 'openrouter'
            ? this.getNodeParameter('openRouterBaseUrl', 0, 'https://openrouter.ai/api/v1')
            : 'https://openrouter.ai/api/v1';
        const ollamaHost = llmProvider === 'ollama'
            ? this.getNodeParameter('ollamaHost', 0, 'ollama:11434')
            : 'ollama:11434';
        ...
        const callLlm = async (prompt) => {
            if (llmProvider === 'openrouter') {
                if (!openRouterApiKey) {
                    throw new n8n_workflow_1.NodeOperationError(node, 'A provider API key is required when LLM Provider uses the OpenRouter-compatible chat completions API');
                }
                ...
                const text = message.content ??
                    ...
                    (Array.isArray(message.reasoning_details)
                        ...
                        : undefined);
                if (!text) {
                    throw new n8n_workflow_1.NodeOperationError(node, 'LLM provider returned no message content or reasoning text');
                }
                return text;
            }
            const res = await helpers.httpRequest({
        ...
        const isAllowed = (verdict) => verdict.arm === 'allow' || verdict.arm === 'constrain';
        const out = [];
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (!userMessage) {
                ...
                throw new n8n_workflow_1.NodeOperationError(node, `Missing input field "${inputFieldName}" on item ${i}`, { itemIndex: i });
            }
            try {
                const result = await govern({ core, preset: presets.n8n, workflowType, taskQueue: 'n8n' }, async (session) => {
                    ...
                    const redactedInput = pre.guardrailsResult?.redactedInput;
                    const promptToUse = redactedInput?.[0]?.[inputFieldName] ?? userMessage;
                    ...
                    let text;
                    let providerFallback = { enabled: false };
                    try {
                    ...
                    const redactedOutput = post.guardrailsResult?.redactedInput;
 ...
 * the integration.
 */
async function resolveOpenBoxEndpoint(ctx) {
    let normalized;
    try {
        const raw = await ctx.getCredentials('openBoxApi');
        normalized = (0, OpenBoxApi_credentials_1.normalizeOpenBoxCredentials)(raw);
    }
    catch {
    ...
    if (!apiUrl || !apiKey) {
        throw new n8n_workflow_1.NodeOperationError(ctx.getNode(), 'OpenBox is not configured: attach an OpenBox credential, or set OPENBOX_API_URL + OPENBOX_API_KEY environment variables.');
    }
    return { apiUrl, apiKey };
}
function buildBlockedResult(nodeName, verdict, stage, session, pre, post) {
    const fallback = stage === 'input' ? 'Request blocked by governance' : 'Response blocked by governance';
    const reason = verdict.reason ?? fallback;
    const text = stage === 'input'
...
function buildProviderErrorResult(nodeName, error, session, pre) {
