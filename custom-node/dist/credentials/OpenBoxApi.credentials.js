    ...
    properties = [
        {
            ...
            description: 'Base URL of the OpenBox Core API. Should be HTTPS in production unless "Enforce HTTPS" is disabled.',
            required: true,
        },
        {
            displayName: 'API Key',
        ...
        },
        {
            ...
            type: 'string',
            default: '',
            description: 'Optional. Forwarded as the X-OpenBox-Organization header to scope multi-org accounts.',
        },
        {
            ...
            type: 'string',
            default: '',
            description: 'Optional. Forwarded as the X-OpenBox-Project header to scope project-level governance rules.',
        },
        {
            ...
            default: 'production',
            description: 'Must match the environment the agent key was issued for. Use "Production" for keys from core.openbox.ai. Forwarded as X-OpenBox-Environment.',
        },
        {
            ...
            description: 'HTTP request timeout in milliseconds. Defaults to 35s; sits slightly above core\'s 30s workflow execution deadline so server-side timeouts surface their real error message.',
        },
        {
        ...
        },
        {
            ...
            type: 'boolean',
            default: true,
            description: 'Reject non-HTTPS OpenBox URLs at request time. Disable only for local development.',
        },
        {
    ...
    authenticate = {
        ...
        properties: {
            headers: {
                ...
                'X-OpenBox-Organization': '={{$credentials.organizationId ? $credentials.organizationId : undefined}}',
                'X-OpenBox-Project': '={{$credentials.projectId ? $credentials.projectId : undefined}}',
...
function normalizeOpenBoxCredentials(raw) {
    ...
    if (enforceHttps && !/^https:\/\//i.test(url)) {
        throw new Error(`OpenBox URL must use HTTPS when "Enforce HTTPS" is enabled. Got: ${url}`);
    }
    const env = raw.environment || 'production';
    const failPolicy = raw.failPolicy || 'fail_closed';
    const timeoutCandidate = Number(raw.timeoutMs);
    const timeoutMs = Number.isFinite(timeoutCandidate) && timeoutCandidate > 0 ? timeoutCandidate : 35000;
    return {
        openboxUrl: url,
        apiKey,
