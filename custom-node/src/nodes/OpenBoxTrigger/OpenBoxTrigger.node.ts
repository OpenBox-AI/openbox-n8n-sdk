...
type OpenBoxEventType = (typeof SUPPORTED_EVENTS)[number];
 ...
 */
export class OpenBoxTrigger implements INodeType {
  description: INodeTypeDescription = {
    ...
    description:
      'Starts a workflow when OpenBox emits an event (approval resolved, policy changed, trust tier changed, alert created, ...).',
    ...
    properties: [
      {
        displayName:
          'Copy the production webhook URL above and register it under "Webhooks" in the OpenBox console. The credential\'s "Webhook Signing Secret" must match the secret OpenBox uses to sign payloads.',
        name: 'setupNotice',
        type: 'notice',
        default: '',
      },
      {
        ...
        description:
          'Only events of the selected types will trigger the workflow. Other events return 200 OK and are silently dropped.',
      ...
      },
      {
        ...
        options: [
          ...
          },
          {
            ...
            default: 0,
            description:
              'Only trigger when the event\'s risk_score is at least this value (0–1 range).',
          },
        ],
      },
      {
        ...
        options: [
          ...
          },
          {
            ...
            value: 'optional',
            description:
              'Verify when present, but accept unsigned requests. Use only behind a trusted gateway.',
          },
          {
      ...
      },
      {
        ...
        default: 300,
        description:
          'Reject requests whose X-OpenBox-Timestamp differs from the server clock by more than this. Set to 0 to disable replay protection.',
      },
    ],
  };
  ...
  async webhook(this: IWebhookFunctions): Promise<IWebhookResponseData> {
    ...
    const events = this.getNodeParameter('events', []) as OpenBoxEventType[];
    const filters = this.getNodeParameter('filters', {}) as IDataObject;
    const signatureMode = this.getNodeParameter(
      'signatureMode',
      'required',
    ) as 'required' | 'optional' | 'disabled';
    const timestampTolerance =
      (this.getNodeParameter('timestampToleranceSeconds', 300) as number) || 0;

    const credsRaw = await this.getCredentials('openBoxApi');
    let credentials: OpenBoxCredentials;
    try {
      credentials = normalizeOpenBoxCredentials(credsRaw);
    } catch (err) {
      throw new NodeOperationError(
        this.getNode(),
        `Invalid OpenBox credential: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
    ...
    if (signatureMode !== 'disabled') {
      if (!credentials.webhookSecret) {
        throw new NodeOperationError(
          this.getNode(),
          'Webhook signing secret is missing on the OpenBox credential. Either add one or set Signature Verification to "Disabled".',
        );
      }
 ...
 */
function readRawBody(_ctx: IWebhookFunctions, body: unknown): string {
...
export interface ParsedEvent {
  type: OpenBoxEventType;
  data: IDataObject;
}

/** Exported for unit tests. */
export function parseEvent(body: unknown): ParsedEvent | undefined {
  ...
  const rawType = (obj.event ?? obj.type ?? obj.event_type) as string | undefined;
  if (!rawType) return undefined;
  const type = rawType as OpenBoxEventType;
  const data = (obj.data as IDataObject) ?? obj;
  return { type, data };
}

/** Exported for unit tests. */
export function matchesFilters(data: IDataObject, filters: IDataObject): boolean {
  const approvalId = filters.approvalId as string | undefined;
  if (approvalId && data.approval_id !== approvalId && data.approvalId !== approvalId) {
    return false;
  }
  const workflowId = filters.workflowId as string | undefined;
  if (workflowId && data.workflow_id !== workflowId && data.workflowId !== workflowId) {
    return false;
  }
  const agentId = filters.agentId as string | undefined;
  if (agentId && data.agent_id !== agentId && data.agentId !== agentId) {
    return false;
  }
  const minRisk = filters.minRiskScore as number | undefined;
  if (typeof minRisk === 'number' && minRisk > 0) {
    const risk = (data.risk_score ?? data.riskScore) as number | undefined;
    if (typeof risk !== 'number' || risk < minRisk) return false;
  }
  return true;
}

function rejectWith(status: number, message: string): IWebhookResponseData {
  return {
    webhookResponse: {
      status,
